...
class DirectMySQLdbConnection(Connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        client_version = tuple(
            [int(n) for n in MySQLdb.connections._mysql.get_client_info().split('.')[:2]])
        ...
        if use_unicode:
            self.converter[FIELD_TYPE.STRING].insert(
                -1, (None, string_decoder))
            self.converter[FIELD_TYPE.VAR_STRING].insert(
                -1, (None, string_decoder))
