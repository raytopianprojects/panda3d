...
class ProtoPaletteUI(wx.Panel):
    def __init__(self, parent, editor):
        ...
        self.tree = PaletteTreeCtrl(
            self, treeStyle=wx.TR_EDIT_LABELS | wx.TR_DEFAULT_STYLE, rootName="Proto Objects")
    ...
    def populate(self):
        ...
        ), self.palette.rootName, self.palette.dataStruct, dataStructKeys)
        ...
        if item != self.tree.GetRootItem():
            ...
            if self.tree.traverse(self.tree.GetRootItem(), newLabel) is None:
                ...
                if isinstance(self.editor.protoPalette.findItem(oldLabel), ObjectBase):
                    event.Veto()
                    wx.MessageBox("Only groups allowed to be renamed",
                                  self.editorTxt, wx.OK | wx.ICON_EXCLAMATION)
                elif not self.editor.protoPalette.rename(oldLabel, newLabel):
                    event.Veto()
                    wx.MessageBox("Label '%s' is not allowed" % newLabel,
                                  self.editorTxt, wx.OK | wx.ICON_EXCLAMATION)
            else:
                event.Veto()
                wx.MessageBox("There is already an item labled '%s'" %
                              newLabel, self.editorTxt, wx.OK | wx.ICON_EXCLAMATION)
        else:
            event.Veto()
            wx.MessageBox("'%s' renaming is not allowed" %
                          self.tree.rootName, self.editorTxt, wx.OK | wx.ICON_EXCLAMATION)
    ...
    def AquireFile(self, filename):
        ...
        newItem = self.tree.AppendItem(
            self.editor.ui.protoPaletteUI.tree.root, name)
    ...
    def addNewItem(self, result):
        if len(result) == 2:
            ...
            itemData = ObjectBase(name=result[0], model=result[1], anims=[
                                  result[2]], actor=True)
        else:
            return
