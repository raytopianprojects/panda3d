...
if not options.license:
    try:
        import pandac
        options.license = os.path.join(os.path.dirname(
            os.path.dirname(pandac.__file__)), "LICENSE")
        ...
        if dfilelower not in dependentFiles:
            if dfilelower.startswith('msvc') or \
               ...
               dfilelower.startswith('python'):
                ...
                # Also recurse.
                addDependencies(path, pathname, file,
                                pluginDependencies, dependentFiles)
...
def makeCabFile(ocx, pluginDependencies):
    ...
    if options.spc and options.pvk:
        ...
        cmd = '"%s" -spc "%s" -k "%s" "%s"' % (
            signcode, options.spc, options.pvk, cabFilename)
        print(cmd)
        result = subprocess.call(cmd)
        if result:
            sys.exit(result)
...
def makeInstaller():
    ...
    if sys.platform == "darwin":
        ...
        if not os.path.exists(tmproot):
            os.makedirs(tmproot)
        dst_npapi = os.path.join(
            tmproot, "Library", "Internet Plug-Ins", npapi)
        dst_panda3d = os.path.join(tmproot, "usr", "local", "bin", panda3d)
        dst_panda3dapp = os.path.join(tmproot, "Applications", panda3dapp)
        if not os.path.exists(dst_npapi):
            os.makedirs(os.path.dirname(dst_npapi))
        if not os.path.exists(dst_panda3d):
            os.makedirs(os.path.dirname(dst_panda3d))
        if not os.path.exists(dst_panda3dapp):
            os.makedirs(os.path.dirname(dst_panda3dapp))
        ...
        if options.license:
            shutil.copyfile(options.license, os.path.join(
                tmpresdir, "License.txt"))
