...
def getCirclePoints(segCount, centerX, centerY, radius, wideX=1.0, wideY=1.0):
    ...
    for seg in range(0, segCount):
        coordX = wideX * \
            (circleX(((pi * 2.0) * float(float(seg) / float(segCount))),
             radius, centerX, centerY))
        coordY = wideY * \
            (circleY(((pi * 2.0) * float(float(seg) / float(segCount))),
             radius, centerX, centerY))
        returnShape.append((coordX, coordY, 1))

    coordX = wideX * \
        (circleX(((pi * 2.0) * float(0 / segCount)), radius, centerX, centerY))
    coordY = wideY * \
        (circleY(((pi * 2.0) * float(0 / segCount)), radius, centerX, centerY))
    returnShape.append((coordX, coordY, 1))
    return returnShape
...
def addCircle(attachNode, vertexCount, radius, color=Vec4(1.0, 1.0, 1.0, 1.0), centerColor=None, layer=0):
    ...
    targetCircleColorWriter.addData4f(
        centerColor[0], centerColor[1], centerColor[2], centerColor[3])
