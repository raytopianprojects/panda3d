...
class WxSlider(wx.Slider):
    def __init__(self, parent, id, value, minValue, maxValue,
        ...
        intVal = 100.0 / (self.maxValue - self.minValue) * \
            (value - self.minValue)
        ...
        if style & wx.SL_HORIZONTAL:
            ...
            if style & wx.SL_LABELS:
                ...
                wx.StaticText(parent, -1, strMaxValue,
                              (pos[0] + size[0] - len(strMaxValue) * 8, pos[1]))
                strValue = "%.2f" % value
                self.textValue = wx.TextCtrl(parent, -1, strValue,
                                             (pos[0] + size[0] / 2 -
                                              textSize[0]/2, pos[1]), textSize,
        ...
        wx.Slider.__init__(self, parent, id, intVal, intMin,
                           intMax, newPos, size, style=newStyle)
        self.Disable()

    def GetValue(self):
        # overriding wx.Slider.GetValue()
        # return (wx.Slider.GetValue(self) * (self.maxValue - self.minValue) / 100.0 + self.minValue)
        # [gjeon] since the value from the slider is not as precise as the value entered by the user
    ...
    def onChange(self, event):
        ...
        floatVal = wx.Slider.GetValue(
            self) * (self.maxValue - self.minValue) / 100.0 + self.minValue
    ...
    def onEnter(self, event):
        ...
        if self.textValue is None:
            return
        intVal = 100.0 / (self.maxValue - self.minValue) * \
            (float(self.textValue.GetValue()) - self.minValue)
        wx.Slider.SetValue(self, intVal)
